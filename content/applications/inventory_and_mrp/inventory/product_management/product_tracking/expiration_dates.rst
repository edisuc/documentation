...
.. tip::
   ...
   :guilabel:`Display Lots & Serial Numbers on Delivery Slips` and to :guilabel:`Display Lots &
   Serial Numbers on Invoices` (under the :guilabel:`Valuation` section). Activating these features
   helps with end-to-end traceability, making it easier to manage product recalls, identify batches
   of faulty products, and more.
...
create a purchase order, go to the :menuselection:`Purchase` app and click :guilabel:`New` to
...
click :guilabel:`Confirm Order`. This converts the |RFQ| into a purchase order.
...
.. important::
   ...
   number for the ordered products. The |RFQ| cannot be validated without an assigned lot or serial
   number.
...
Date` parameter from the drop-down menu.
...
all of the search filters from the :guilabel:`Search...` bar on the :guilabel:`Lots/Serial Numbers`
dashboard.
